...
import no.fdk.fdk_informationmodel_harvester.model.UpdateSearchMessage
...
class RabbitMQPublisher(private val template: RabbitTemplate) {
    fun send(dbId: String?) {
        try {
            template.convertAndSend("harvests","newinformationmodels.harvester.UpdateSearchTrigger", UpdateSearchMessage(dbId))
